base64 -i img.svg; //to obtain the encoded format

HappyMood:
data:image/svg+xml;base64,

SadMood:
data:image/svg+xml;base64,


How to run
First:
make anvil //run the anvil chain
make deployMood //runs the script of MoodNft

Second:
"Mint the Nft"
cast send mood(contract address) "mintNft()" --private-key 59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d --rpc-url http://localhost:8545

"flips the mood"
cast send mood(contract address) "flipMood(uint256)" 0 --private-key 59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d --rpc-url http://localhost:8545
